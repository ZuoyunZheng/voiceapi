...
async def websocket_tts(
    ...
    interrupt: bool = Query(
        True,
        title="Interrupt",
        description="Interrupt the current TTS stream when a new text is received.",
    ),
    sid: int = Query(
        0, title="Speaker ID", description="The ID of the speaker to use for TTS."
    ),
    ...
    speed: float = Query(
        1.0, title="Speed", description="The speed of the generated audio."
    ),
    split: bool = Query(
        True, title="Split", description="Split the text into sentences."
    ),
):
    await websocket.accept()
...
async def tts_generate(req: TTSRequest):
    ...
    if not tts_stream:
        raise HTTPException(status_code=500, detail="failed to start TTS stream")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--asr-provider", type=str, default="cpu", help="asr provider, cpu or cuda"
    )
    parser.add_argument(
        "--tts-provider", type=str, default="cpu", help="tts provider, cpu or cuda"
    )

    parser.add_argument("--threads", type=int, default=2, help="number of threads")

    parser.add_argument(
        "--models-root", type=str, default=models_root, help="model root directory"
    )
    ...
    parser.add_argument(
        "--asr-lang", type=str, default="zh", help="ASR language, zh, en, ja, ko, yue"
    )

    parser.add_argument(
        "--tts-model",
        type=str,
        default="vits-melo-tts-zh_en",
    ...
    if args.tts_model == "vits-melo-tts-zh_en" and args.tts_provider == "cuda":
        logger.warning("vits-melo-tts-zh_en does not support CUDA fallback to CPU")
